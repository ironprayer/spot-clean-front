...
export default function CleanupCompletePage() {
  ...
  const [trashUnit, setTrashUnit] = useState<TrashUnit>("개");
  const [notes, setNotes] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const elapsedMs = cleanupStartedAt ? Date.now() - cleanupStartedAt : 0;
  ...
  const parsedCount = parseFloat(trashCount);
  const canSubmit =
    !submitting && trashCount.trim() !== "" && !isNaN(parsedCount) && parsedCount > 0;
  ...
  const handleSubmit = async () => {
    if (!canSubmit || !activeCleanup) return;
    setSubmitting(true);
    try {
      // TODO: FormData API call with photos, count, unit, notes
      console.log("완료 제출:", {
        reportId: activeCleanup.id,
        trashCount: parsedCount,
        trashUnit,
        notes,
      ...
      <header className="bg-white border-b border-gray-100 px-4 pt-safe shrink-0">
        <div className="flex items-center gap-3 py-4">
          ...
          <div className="flex-1">
            <h1 className="text-base font-bold text-gray-900">줍기 완료</h1>
            {cleanupStartedAt && (
              <p className="text-xs text-gray-400 mt-0.5">경과 시간 {formatElapsed(elapsedMs)}</p>
            )}
          </div>
          {activeCleanup && (
            <span className="text-xs text-gray-400">신고 #{activeCleanup.id}</span>
          )}
        </div>
      </header>
        ...
        <div className="p-4 flex flex-col gap-4 pb-6">
          ...
          <section className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h2 className="text-sm font-semibold text-gray-800">After 사진</h2>
                <p className="text-xs text-gray-400 mt-0.5">줍고 난 후 현장 사진 (선택)</p>
              </div>
            ...
            <p className="text-xs text-gray-400 mt-3">
              GPS 정보가 있는 현재 위치 반경 300m 이내의 사진만 업로드됩니다.
            </p>
          </section>
            ...
            <section className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <div className="flex flex-col gap-2">
                {photos.map((photo, i) => (
                  ...
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={photo.preview} alt="" className="w-10 h-10 rounded-lg object-cover shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-gray-700 truncate">{photo.file.name}</p>
                      {photo.gps ? (
                      ...
                      {photo.distKm !== null && (
                        <p className={`text-[11px] font-semibold mt-0.5 ${photo.error ? "text-red-600" : "text-green-600"}`}>
                          현재 위치에서 {(photo.distKm * 1000).toFixed(0)}m
                        </p>
                      )}
                      {photo.error && (
                        <p className="text-[11px] text-red-500 mt-0.5 leading-tight">{photo.error}</p>
                      )}
                    </div>
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${photo.error ? "bg-red-100" : "bg-green-100"}`}>
                      {photo.error ? (
          ...
          <section className="bg-white rounded-2xl p-4 shadow-sm">
            <h2 className="text-sm font-semibold text-gray-800 mb-3">
              주운 쓰레기 수량 <span className="text-red-400">*</span>
            </h2>
            <div className="flex gap-2">
              <input
                ...
                placeholder="0"
                className="flex-1 text-sm text-gray-800 placeholder-gray-400 border border-gray-100 bg-gray-50 rounded-xl px-3 py-2.5 focus:outline-none focus:border-green-400 focus:bg-white transition-colors"
              />
              {/* Unit toggle */}
              <div className="flex rounded-xl overflow-hidden border border-gray-100 bg-gray-50 shrink-0">
                {(["개", "L"] as TrashUnit[]).map((unit) => (
                  <button
                    key={unit}
                    onClick={() => setTrashUnit(unit)}
            ...
            {trashCount && !isNaN(parsedCount) && parsedCount > 0 && (
              <p className="text-xs text-green-600 mt-2 font-medium">
                {parsedCount} {trashUnit} 수거
              </p>
            )}
          </section>
          ...
          {/* ── Notes ── */}
          <section className="bg-white rounded-2xl p-4 shadow-sm">
            <h2 className="text-sm font-semibold text-gray-800 mb-3">기록 · 느낀점</h2>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="오늘 줍기 어땠나요? 기억하고 싶은 것들을 자유롭게 남겨 보세요."
              rows={4}
            ...
            />
            <p className="text-xs text-gray-400 mt-1 text-right">{notes.length}자</p>
          </section>
